\documentclass[a4paper,punct,space,heading=true]{ctexrep}
...
\pagestyle{empty}
\chead{\zihao{5}中国民航大学本科毕业设计论文}% 使用rhead 定义右侧上方页眉
\rhead{~}% 设置左侧页眉为空
\lhead{~}% 设置右侧页眉为空
...
\CTEXoptions[contentsname={\hei\zihao{3}目~~录}]
...
\newline
\newline
\indent\zihao{-4}\hei{关键词：}\zihao{-4}\song 毕业设计（论文）；撰写细则；行文格式
\newpage

\setlength{\headsep}{1.224cm}
\begin{center}
\zihao{3}\hei Abstract
\end{center}
\vspace{21pt}
\zihao{-4}
...
\newline
\newline
\indent\zihao{-4}\textbf{Key Words: }\zihao{-4}\songti （与上文之间空一行，内容应与中文"关键词"一致。词间用英文分号";"间隔，末尾不加标点。）
...
\chapter{绪论}
\setlength{\baselineskip}{20pt}
一篇完整的毕业设计或论文\cite{EDWARDS200130,janson_thomason_2008}通常由题目、摘要、目录、正文、结论、参考文献、致谢、附录等几部分构成，除外语类专业外一律采用汉语撰写。
...
\subfloat[图b标题]{\includegraphics[width=0.4\textwidth]{fig21b.png}\label{fig1}}
\caption{跟随冷源对温度场的影响}
\end{figure}
